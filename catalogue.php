            ...
            <div class="catalogue-games">
                <?php
                    ...
                    if(isset($_GET["search"])){
                        ...
                        $query = ('SELECT * FROM articles WHERE Nom LIKE :Value'); //Le :User permet de faire référence à la valeur User
                        $data = [':Value' => $value];
                        $prep = $dbh->prepare($query);	//Vérifie la commande sql
                        $result = $prep->execute($data); //Execute la commande
                        ...
                        foreach($results as $row){
                            echo "<div class='game'>";
                                $requete2 = "SELECT * FROM image WHERE ID_JEU = $row[ID_JEU]";
                                ...
                                if(mysqli_num_rows($result2) == 0 ){
                                    ...
                                    echo '<a href ="detailled.php?id='.$row["ID_JEU"].'"><img class="game-img" src="data:image/png;base64,' . base64_encode($myrow2["Lien"]) . '" /> </a>';
                                }
                                ...
                                if (strlen($row["Nom"])> 30){
                                    ...
                                    echo("<span id='game-name'>$row[Nom]</span>");
                                }
                                    echo("<span id='game-price'>$row[Prix]€</span>");
                                echo("</div>");
